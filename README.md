# Architecting Agentic Sandpits

> Effect Systems for Agentic Development — 15-minute lightning talk

**[Watch on YouTube](https://youtu.be/JaLAvoyjwoQ)**

**[View slides](https://drshade.github.io/talk_agentic_effect_systems/)**

## Contents

- `slides/slides.md` — Marp slide deck (8 slides)
- `talk/talk-notes.md` — full speaker notes (~15 min)

## Developing

```bash
npm install
npm run slides:watch   # live preview at localhost
```

## Building

```bash
npm run slides         # slides/slides.html
npm run slides:pdf     # slides/slides.pdf
```
